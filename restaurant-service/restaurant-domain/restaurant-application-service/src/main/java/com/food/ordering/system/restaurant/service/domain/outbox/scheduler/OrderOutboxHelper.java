...
public class OrderOutboxHelper {
    ...
    private void save(OrderOutboxMessage orderPaymentOutboxMessage) {
        OrderOutboxMessage response = orderOutboxRepository.save(orderPaymentOutboxMessage)
                .orElseThrow(()->{
                    log.error(("OrderOutboxMessage 를 저장하는데 실패했습니다. OrderOutboxMessage: {}"), orderPaymentOutboxMessage.getId());
                    return new RestaurantDomainException(String.format("OrderOutboxMessage 를 저장하는데 실패했습니다. OrderOutboxMessageId: %s", orderPaymentOutboxMessage.getId()));
                });
        log.info("OrderOutboxMessage 를 저장했습니다.: {}", orderPaymentOutboxMessage.getId());
    }
